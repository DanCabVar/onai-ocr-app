...
export interface InferFromSamplesJobStartResponse {
  jobId: string
  status: 'processing'
}

export interface InferFromSamplesJobStatusResponse {
  jobId: string
  status: 'processing' | 'completed' | 'failed'
  step: string
  progress: number
...
class DocumentTypeInferenceService {
  ...
  async getInferenceJobStatus(
    jobId: string,
  ): Promise<InferFromSamplesJobStatusResponse> {
    const response = await fetch(`${API_BASE_URL}/document-types/jobs/${jobId}`, {
      method: 'GET',
    ...
    return (await response.json()) as InferFromSamplesJobStatusResponse
  }

  async startInferenceJob(
    files: File[],
    uploadSamples: boolean = false,
  ): Promise<InferFromSamplesJobStartResponse> {
    ...
    return (await response.json()) as InferFromSamplesJobStartResponse
  }

  async waitForInferenceJob(
    jobId: string,
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      const job = await this.getInferenceJobStatus(jobId)
  ...
  async inferFromSamples(
    ...
    const job = await this.startInferenceJob(files, uploadSamples)
    ...
    return this.waitForInferenceJob(job.jobId, onProgress)
  }
