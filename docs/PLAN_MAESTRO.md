...
| T22 | Alta | Remediación de vulnerabilidades residuales post-SPEC-13 | En revisión | `docs/03_specs/active/SPEC-22_remediacion_vulnerabilidades_residuales.md` | Residual high en lodash depende de parche npm >=4.18.0 o reemplazo de dependencia raíz | Remediación ejecutada + evidencia en `docs/04_trabajo/T22_remediacion_vulnerabilidades_residuales/README.md` | Monitorear release parcheado de lodash y cerrar residual con upgrade estructural |
| T23 | Alta | Estandarizar configuración ESLint (frontend + backend) | En trabajo por agente | `docs/03_specs/active/SPEC-23_estandarizar_eslint_config.md` | Debe evitar refactor masivo y partir con baseline pragmático | Config ESLint versionada + CI ejecutando lint obligatorio en ambos proyectos | Implementación funcional en rama `codex/spec-23-estandarizar-eslint-config`; mantener `deploy/all-features` en docs-only hasta validación/merge |
| T24 | Alta | RAG h�brido sobre Markdown grafo (Obsidian-ready) | En trabajo por agente | `docs/03_specs/active/SPEC-24_rag_hibrido_markdown_grafo_obsidian.md` | Depende de outputs de SPEC-21 y control estricto de aislamiento por tenant | Retrieval h�brido con trazabilidad de fuentes desde respaldos Markdown | Implementaci�n funcional en rama `codex/spec-24-rag-hibrido-markdown-grafo-obsidian` (incluye `query-debug` y scoring tipo/fecha); en `deploy/all-features` se mantiene commit docs-only de estado y trazabilidad |
