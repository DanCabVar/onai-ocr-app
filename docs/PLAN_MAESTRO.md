...
| T24 | Alta | RAG híbrido sobre Markdown grafo (Obsidian-ready) | En trabajo por agente | `docs/03_specs/active/SPEC-24_rag_hibrido_markdown_grafo_obsidian.md` | Depende de outputs de SPEC-21 y control estricto de aislamiento por tenant | Retrieval híbrido con trazabilidad de fuentes desde respaldos Markdown | Implementación funcional en rama `codex/spec-24-rag-hibrido-markdown-grafo-obsidian`; en `deploy/all-features` se mantiene commit docs-only de estado y trazabilidad |
