...
| T19 | Alta | Revision de separacion por tenant | En revision | `docs/03_specs/active/SPEC-19_revision_separacion_tenant.md` | Complementa auditoria de seguridad de T08; hardening RAG/RLS/storage ya auditado, corregido hallazgo de inferencia cross-tenant y validado en QA que un usuario nuevo no ve tipos de otro tenant | Informe de auditoria + hardening + pruebas de fuga cruzada | Ejecutar validaciones cruzadas restantes en QA (documentos/chat si aplica) y cerrar evidencia final |
| T20 | Media | Mejorar agente RAG con grafo Neo4j | En revisión | docs/03_specs/active/SPEC-20_mejorar_agent_rag_neo4j.md | Pendiente validación benchmark calidad/latencia y criterios de fallback | Diseno + PoC de consulta hibrida SQL+grafo | Ejecutar benchmark reproducible, consolidar evidencia y preparar merge controlado |
