...
| T28 | Alta | Calidad y consistencia del chatbot documental | Lista para deploy | `docs/03_specs/active/SPEC-28_calidad_chatbot_documental.md` | Desarrollo determinístico completo (Fases 1-3): capa intención/resolución, anclaje por filename, depuración de entidad, agregados sin entidad, statement de aclaración T28-015, trazabilidad strategy/intent + árbol de fallback explícito; 42 tests + build OK en `dev-local`. Único caso adversarial T28-014 se deja a ruta generativa por diseño. Resta solo validación en vivo (Fase 4), que es en QA por definición | Estrategia robusta de chatbot + benchmark versionado + regresiones automáticas | Promover `dev-local → dev → qa` y correr el benchmark QA en vivo (`benchmark_v1.csv`), marcar casos `draft`→validado |
