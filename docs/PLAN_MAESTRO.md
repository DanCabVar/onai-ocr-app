...
| T20 | Media | Mejorar agente RAG con grafo Neo4j | En revisi?n | `docs/03_specs/active/SPEC-20_mejorar_agent_rag_neo4j.md` | Pendiente benchmark de calidad/latencia en QA y validaci?n controlada del fallback SQL; la robustez transversal del chatbot frente a documentos nuevos se desacopla en `T28` | PoC h?brida SQL+grafo integrada en `dev-local` detr?s de feature flag con build OK | Consolidar benchmark reproducible de T20 y mover mejoras generales del chatbot a `T28` |
...
| T28 | Alta | Calidad y consistencia del chatbot documental | Disponible para agente | `docs/03_specs/active/SPEC-28_calidad_chatbot_documental.md` | Nace de validaci?n QA de `T20`; requiere benchmark sem?ntico, cat?logo de intenciones y resoluci?n robusta de campos para documentos actuales y nuevos | Estrategia robusta de chatbot + benchmark versionado + regresiones autom?ticas | Priorizar dise?o de intenciones/campos y armar benchmark QA reusable |
