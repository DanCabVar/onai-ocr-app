...
| T26 | Alta | Migraciones DB seguras en CI/CD | En revisión | `docs/03_specs/active/SPEC-26_db_migrations_ci_cd.md` | Implementación base lista en branch: configuración TypeORM centralizada, scripts `db:*`, workflow QA con migrate y workflow prod con backup+migrate; falta validación en ambientes reales | Migraciones versionadas, `synchronize` desactivado en QA/prod, backup prod y runbook DB | Ejecutar validación real en QA (`db:show`/`db:migrate`) y confirmar el flujo productivo canónico antes del cierre |
