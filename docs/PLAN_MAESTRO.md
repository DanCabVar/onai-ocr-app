...
| T20 | Media | Mejorar agente RAG con grafo Neo4j | En revisi?n | `docs/03_specs/active/SPEC-20_mejorar_agent_rag_neo4j.md` | Pendiente benchmark de calidad/latencia en QA y validaci?n controlada del fallback SQL | PoC h?brida SQL+grafo integrada en `dev-local` detr?s de feature flag con build OK | Promover `dev -> qa`, ejecutar benchmark reproducible y consolidar evidencia de fallback seguro |
